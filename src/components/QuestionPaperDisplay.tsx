      ...
      <Card className="printable-area shadow-2xl rounded-lg border-2 border-primary/20 bg-white text-black" id="question-paper">
              ...
              <div className="space-y-4 text-sm">
                {questions.mcqs.map((questionText, index) => (
                  <div key={`mcq-${index}`} className="pb-2">
                    ...
                    <div className="flex flex-wrap gap-3 items-center ml-4 mt-2">
                      {['a', 'b', 'c', 'd'].map(option => (
                        <div key={option} className="flex items-center justify-center border border-gray-400 rounded-md px-4 py-2 min-w-[50px] text-center text-gray-600 cursor-default">
                          {option}
                        </div>
                      ))}
                    </div>
        ...
        <div className="p-4 border-t border-gray-300 text-center text-xs text-gray-500">
            <div className="w-full">
                <p className="mb-1 font-semibold">*** END OF QUESTION PAPER ***</p>
